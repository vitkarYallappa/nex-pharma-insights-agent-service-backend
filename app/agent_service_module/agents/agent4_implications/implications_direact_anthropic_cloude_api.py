"""
Direct Anthropic Claude API integration for Agent 4 Implications
Provides high-performance direct API access to Anthropic's Claude models
"""
...
class AnthropicAPI:
    """Direct Anthropic Claude API client for implications generation"""

    def __init__(self):
        self.client = anthropic.AsyncAnthropic(api_key=settings.ANTHROPIC_API_KEY)
        self.model = getattr(settings, 'ANTHROPIC_MODEL', 'claude-3-sonnet-20240229')
        self.max_tokens = getattr(settings, 'ANTHROPIC_MAX_TOKENS', 4000)
        self.temperature = getattr(settings, 'ANTHROPIC_TEMPERATURE', 0.2)
    ...
    async def generate_strategic_implications(
        ...
        """Generate strategic implications using direct Anthropic API"""
        try:
            ...
            for impl_data in result.get("strategic_implications", []):
                try:
                    implication = StrategicImplication(
                        implication_id=f"anthropic_impl_{int(datetime.utcnow().timestamp())}_{len(implications)}",
            ...
            logger.info(f"Generated {len(implications)} strategic implications via Anthropic")
            return implications

        except Exception as e:
            logger.error(f"Anthropic implications generation failed: {str(e)}")
            return []
    ...
    async def generate_scenario_planning(
        ...
        """Generate scenario planning using direct Anthropic API"""
        try:
            ...
            if not result:
                logger.error("Failed to parse JSON from Anthropic scenario response")
                return []
            ...
            for scenario_data in result.get("scenario_planning", []):
                try:
                    scenario = ScenarioOutcome(
                        scenario_id=f"anthropic_scenario_{int(datetime.utcnow().timestamp())}_{len(scenarios)}",
            ...
            logger.info(f"Generated {len(scenarios)} scenario outcomes via Anthropic")
            return scenarios

        except Exception as e:
            logger.error(f"Anthropic scenario planning generation failed: {str(e)}")
            return []
    ...
    async def generate_stakeholder_analysis(
        ...
        """Generate stakeholder-specific analysis using direct Anthropic API"""
        try:
            ...
            logger.info(f"Generated Anthropic stakeholder analysis for {len(stakeholder_analyses)} roles")
            return stakeholder_analyses

        except Exception as e:
            logger.error(f"Anthropic stakeholder analysis generation failed: {str(e)}")
            return {}
    ...
    async def synthesize_insights(
        ...
        """Create executive synthesis using direct Anthropic API"""
        try:
            ...
            if result:
                ...
                logger.info("Generated Anthropic executive synthesis")
                return synthesis
            else:
                logger.error("Failed to parse Anthropic synthesis response")
                return {}

        except Exception as e:
            logger.error(f"Anthropic synthesis generation failed: {str(e)}")
            return {}

    async def generate_priority_matrix(self, implications: List[StrategicImplication]) -> PriorityMatrix:
        """Generate priority matrix using business logic"""
        try:
